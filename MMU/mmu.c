...
void allocate_memory(list_t * freelist, list_t * alloclist, int pid, int blocksize, int policy) {
  ...
  // Get the block to allocate from free list
  block_t *blk = list_remove_at_index(freelist, index);
    
  // Set the block's PID
  blk->pid = pid;
    
  // Calculate original block's end before modification
  int original_end = blk->end;
    
  // Set the block's end to match requested blocksize
  blk->end = blk->start + blocksize - 1;
    
  // Add the block to the allocation list in ascending order by address
  list_add_ascending_by_address(alloclist, blk);
    
  // Handle fragment if there's remaining memory
...
void deallocate_memory(list_t * alloclist, list_t * freelist, int pid, int policy) { 
  ...
  // Remove the block from allocation list
